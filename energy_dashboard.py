...
# The datatset
#os.chdir(r'C:\Users\makar\OneDrive\Desktop\UE_Applied_Sciences\2 semester\Data Visualization\F P')
...
df_top_prod_5 = df_top_prod.sort_values(by='amount of energy', ascending=False).reset_index(drop=True)[:5]

# Barplot of top 5 countries by the amount of energy produced
with col2:
    st.subheader(f'Top 5 energy producers in {continent}')
    fig = px.bar(df_top_prod_5, x='country', y='amount of energy',
                 color='amount of energy', color_continuous_scale='viridis')
    st.plotly_chart(fig, use_container_width=True)

# Download the selected data
csv = df_top_prod.to_csv(index=False).encode('utf-8')
st.download_button('Download Data for Selected Year and Region', data=csv, file_name='SelectedData.csv', mime='text/csv',
